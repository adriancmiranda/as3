package com.am.media {
	...
	import com.am.events.MediaEvent;
	import com.am.net.Stream;
	...
	public class Video extends flash.media.Video {
		...
		public function seek(second:Number):void {
			this._stream.seek(clamp(second, 0, this._duration));
		}

		public function step(frame:Number, loop:Boolean = false):void {
			if (!this._stream.paused) {
				frame = clamp(Math.abs(frame), MIN_SPEED, MAX_SPEED);
				this._speed = frame;
				this._frames += this._speed;
				this.seek(clamp(0, this._frames, this._duration));
				if (loop) {
					this.stepLoop();
				} else {
					this._frames = clamp(this._frames, 0, this._duration);
				}
			}
		}
		...
		private function stepLoop():void {
			...
			if (this._stream.time <= this._speed) {
				this._frames = this._duration - (this._speed * 1.75) - 0.005;
			}
			if (this._stream.time > this._duration - (this._speed * 1.75) - 0.05) {
				this._frames = this._speed + 0.005;
			}
		}
