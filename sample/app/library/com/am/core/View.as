package com.am.core {
	...
	public final class View {
		...
		private var _prioritize:String;
		...
		public function View(xml:XML, binding:DisplayObject = null) {
			...
			this._preload = bool(xml.@preload);
			this._prioritize = xml.@prioritize.toString();
			...
			trace(this);
			this._branch = BranchHelper.cleanup(this._branch);
			this._level = BranchHelper.getLevel(this._branch);
			if (xml.hasOwnProperty('file')) {
				this._files = this.parseFiles(xml, this);
			}
		...
		private function validateViewNode(node:XML):void {
			...
			if (node == null) {
				...
				throw new AMError(error + node.@id + ' node missing required attribute \'class\'');
			}
			else if ((bool(node.@menu) || bool(node.@landing)) && (node.@title == undefined || !node.@title.length)) {
				throw new AMError(error + node.@id + ' missing required attribute \'title\'');
			}
			else if (node.@window != undefined && !BranchHelper.isValidWindow(node.@window)) {
				throw new AMError(error + node.@id + ' \'window\' attribute contains invalid target \'' + node.@window + '\'');
			}
			else if (node.(descendants('view').@id.contains(node.@id)).length()) {
				throw new AMError(error + node.@id + ' node duplicate');
			}
		}
		...
		private function validateBaseNode(node:XML):void {
			...
			if (node == null) {
				...
				throw new AMError(error + node.@id + ' node missing required attribute \'standard\'');
			}
			else if (INDEX == 1 && node.@mistake == undefined) {
				throw new AMError(error + node.@id + ' node missing required attribute \'mistake\'');
			}
			else if (node.@standard != undefined && !node.view.(@id == node.@standard).length()) {
				throw new AMError(error + node.@id + ' \'standard\' node missing');
			}
			else if (INDEX == 1 && !node.view.(@id == node.@mistake).length()) {
				throw new AMError(error + node.@id + ' \'mistake\' node missing');
			}
		}
		...
		public function getView(value:* = ''):View {
			var view:View;
			var indexBranch:int;
			if (value is String) value = BranchHelper.cleanup(value);
		...
		public function get prioritize():String {
